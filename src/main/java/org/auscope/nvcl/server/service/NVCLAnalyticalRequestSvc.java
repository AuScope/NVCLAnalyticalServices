 ...
 * AnalyticalJobProcessor.
 * At the end, it will create a new message in the JMS status and result queue
 * with status
...
public class NVCLAnalyticalRequestSvc {
	...
	/**
	 * sends an email to the requestor's email address indicating success and
	 ...
	 */
	private void sendResultEmail(AnalyticalJobResultVo messageVo, String jobResultUrl, String jobStartTime,
			String jobEndTime) {
		...
		msgtext = "This is an automated email from the NVCL Analytical Service.\n\nThe analytical job you requested :"
				...
				+ NVCLAnalyticalRequestSvc.config.getMsgTimetoLiveDays()
		...
		sendGridMailService.sendMail(messageVo.getEmail(),
				"NVCL Analytical job='" + messageVo.getJobDescription() + "' result is ready", msgtext);
